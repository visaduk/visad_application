...
function buildCostApplicant(tq, defaults) {
  const base = defaults.cost_applicant ? [...defaults.cost_applicant] : [];
  if (tq.has_credit_card && tq.has_credit_card.toLowerCase() === 'yes' && !base.includes('credit_card')) {
    base.push('credit_card');
  }
...
function mapDbToForm(t, tq, cfg) {
  ...
  const data = {
    ...
    cost_applicant:        buildCostApplicant(tq, defaults),
  };
